...
export default function Minors() {
  ...
  const [minorTex, setminorTex] = useState("");
  ...
  const writeMinor = (matrix, order) => {
    setMatArr();
    let temp = "";
    for (let i = 0; i < order; i++) {
      for (let j = 0; j < order; j++) {
        temp += `M_{${i + 1}${j + 1}} = ${minor(
          matrix,
          order,
          i,
          j
  ...
  const show = () => {
    setminorTex(writeMinor(matrix, order));
    setIsOpen(true);
  };
          ...
          >
            <Modal.Header>
              <Modal.Title id="contained-modal-title-vcenter">
                Minors of the entered matrix are:
              </Modal.Title>
              <button
                type="button"
                className="btn-close"
